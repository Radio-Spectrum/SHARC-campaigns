...
# Options:
#   - "latest": Use only the most recent execution (highest number, e.g., _02 over _01)
...
def parse_features_from_path(path: Path) -> Dict[str, Optional[str]]:
    ...
    s = str(path).lower()
    s_original = str(path)  # Keep original case for execution number extraction
    ...
    # Pattern: _YYYY-MM-DD_NN where NN is the execution number
...
def filter_duplicate_simulations(
    ...
    duplicate_handling : str or int
        How to handle duplicates:
        - "latest": Use only the most recent execution (highest number)
        - "all": Keep all executions
        - int: Use only execution with this specific number
    
    Returns
    -------
    List[Path]
        Filtered list of CSV file paths
    """
...
def plot_ccdf_inr(
    ...
    duplicate_handling : Optional[Union[str, int]]
        How to handle duplicate simulations (same parameters, different execution numbers):
        - "latest": Use only the most recent execution (highest number, default)
        - "all": Plot all executions as separate curves
        - int: Use only execution with this specific number (e.g., 1 for _01, 2 for _02)
        If None, uses DUPLICATE_HANDLING from settings
    """
...
if __name__ == "__main__":
    ...
    curves_to_plot = [
        ...
        #(1600 + distance[0], (1600 + distance[0]), height[0], "one_end", "FIXED", f"To the right, h={height[0]}m, Distance = {distance[0]/1000}Km"),
        #(1600 + distance[1], (1600 + distance[1]), height[0], "one_end", "FIXED", f"To the right, h={height[0]}m, Distance = {distance[1]/1000}Km"),
        #(1600 + distance[2], (1600 + distance[2]), height[0], "one_end", "FIXED", f"To the right, h={height[0]}m, Distance = {distance[2]/1000}Km"),
        ...
        #(1600 + distance[3], -(1600 + distance[3]), height[0], "both_ends", "FIXED", f"Δ = {distance[3]/1000} km"),
         ...
         #(1600 + distance[2], None, height[0], "both_ends", "UNIFORM", f"UNIFORM, h={height[0]}m, Distance = {distance[2]/1000}Km"),
         #(1600 + distance[3], None, height[0], "both_ends", "UNIFORM", f"UNIFORM, h={height[0]}m, Distance = {distance[3]/1000}Km"),
         #(1600 + distance[4], None, height[0], "both_ends", "UNIFORM", f"UNIFORM, h={height[0]}m, Distance = {distance[4]/1000}Km"),
    ...
    # Duplicate simulation handling
    # Options: "latest" (use most recent), "all" (plot all), or int (specific execution number)
    duplicate_handling = "latest"  # By default, always use the latest execution
    # =========================================================
