...
auto_open = False
do_aggregation_plots = True
perc_time = 0.001
protection_criteria = -6
link = "downlink"

exclusions_to_aggregate = {
    "system-3.2110-2200MHz.525km": ["40"],
    "system-4.2110-2200MHz.690km": ["40"],
}
...
def get_compatible_patterns_regex(s1, s2):
    ...
    if num_matches == len(must_match_idxs) and \
        s1_matches[0] != s2_matches[0] and \
            s1_matches[6] != s2_matches[6] and \
                s1_matches[1] in exclusions_to_aggregate[s1_matches[6]] and \
                    s2_matches[1] in exclusions_to_aggregate[s2_matches[6]]:
...
# result_pattern = fr".*_{country}_.*exclusion_{load}load_{link}_.*_.*{imt_scenario}"
...
output_dir_combinations = combinations(ccdf_results, 2)
for r, r2 in output_dir_combinations:
    ...
    if legend is not None:
        ...
        plots = post_processor.generate_ccdf_plots_from_results(
            [r, r2], cutoff_percentage=0.001 / 2
        )
        post_processor.add_plots(plots)
        fig = post_processor.get_plot_by_results_attribute_name('imt_dl_inr', plot_type="ccdf")
        plots_to_save.append((HTMLS_DIR / f"aggregated-{legend.replace(' ', '_')}.html", fig))
        ...
        fig.add_hline(perc_time, line_dash="dash", annotation=dict(
