...
def main():
    ...
    results_sorted = sorted(
        results,
        key=lambda x: (x["ES_Type"], x["Satellite_Distance"], x["Frequency_Offset"])
    )
    
    # Save to CSV
    csv_file = CAMPAIGN_DIR / "protection_margins.csv"
    with open(csv_file, 'w', newline='') as f:
        fieldnames = ["ES_Type", "Satellite_Distance", "Frequency_Offset", 
    ...
    print("=" * 120)
    print(f"{'ES Type':<15} {'Satellite':<12} {'Frequency Offset':<25} {'Threshold':<20} {'Margin (dB)':<15} {'Status':<10}")
    print("-" * 120)
    
    for row in results_sorted:
        print(f"{row['ES_Type']:<15} {row['Satellite_Distance']:<12} {row['Frequency_Offset']:<25} "
